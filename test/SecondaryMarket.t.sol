...
contract SecondaryMarketTest is Test {
    ...
    function _listTicketAfterCreation() private returns(ITicketNFT) {
        ...
        vm.startPrank(bob);
        collection.approve(address(secondaryMarket), 1);
        secondaryMarket.listTicket(address(collection), 1, 1e7);
        vm.stopPrank();
        return collection;
    }
    ...
    function testSuccessfulListTicket() public {
        ...
        emit Listing(bob, address(collection), 1, 1e7);
        secondaryMarket.listTicket(address(collection), 1, 1e7);
        vm.stopPrank();

        assertEq(collection.balanceOf(bob), 0);
        assertEq(collection.balanceOf(address(secondaryMarket)), 1);
        assertEq(collection.holderOf(1), address(secondaryMarket));
        assertEq(secondaryMarket.getHighestBid(address(collection), 1), 1e7);
    }
    ...
    */
    ...
    function testListUsedTicket() public {
        ITicketNFT collection = _createCollectionAndMintOneTicket();
        vm.prank(alice);
        collection.setUsed(1); // Collection creator set the ticket to used
        vm.startPrank(bob);
        collection.approve(address(secondaryMarket), 1);
        vm.expectRevert("The ticket has already expired/been used");
        secondaryMarket.listTicket(address(collection), 1, 1e7);
    ...
    function testSuccessfulSubmitBid() public {
        ITicketNFT collection = _listTicketAfterCreation();
        vm.startPrank(charlie);
        purchaseToken.mint{value: 1e7}(); // This value will be x100
        purchaseToken.approve(address(secondaryMarket), 1e9); 
        vm.expectEmit(true, true, true, true);
        emit BidSubmitted(charlie, address(collection), 1, 1e9, "Charles");
        secondaryMarket.submitBid(address(collection), 1, 1e9, "Charles");
        vm.stopPrank();

        assertEq(secondaryMarket.getHighestBid(address(collection), 1), 1e9);
        assertEq(secondaryMarket.getHighestBidder(address(collection), 1), charlie);
    ...
    function testSubmitBidWithoutApproval() public {
        ...
        secondaryMarket.submitBid(address(collection), 1, 1e8, "Alice");
        vm.stopPrank();
    }
    ...
    */
    function testSubmitBidForUnlistedTicket() public {
        ...
        secondaryMarket.submitBid(address(collection), 1, 1e7, "Alice");
        vm.stopPrank();
    }
    ...
    function testSubmitBidThatIsBellowPrice() public {
        ...
        secondaryMarket.submitBid(address(collection), 1, 1e5, "Alice");
        vm.stopPrank();
    }
    ...
    function testSubmitBidThatIsBellowPreviousMaxBid() public {
        ...
        purchaseToken.mint{value: 1e10}(); 
        purchaseToken.approve(address(secondaryMarket), 1e9); 
        secondaryMarket.submitBid(address(collection), 1, 1e9, "Alice"); 
        vm.stopPrank();
        ...
        secondaryMarket.submitBid(address(collection), 1, 1e8, "Robert"); 
        vm.stopPrank();
    }
    ...
    function testSuccessfulAcceptBid() public {
        ...
        vm.prank(bob);
        vm.expectEmit(true, true, true, true);
        emit BidAccepted(charlie, address(collection), 1, 1e9, "Charles");
        secondaryMarket.acceptBid(address(collection), 1);
    ...
    function testSuccessfulTicketDelisting() public {
        ...
        emit Delisting(address(collection), 1);
        secondaryMarket.delistTicket(address(collection), 1);
        assertEq(collection.balanceOf(address(secondaryMarket)), 0);
        vm.stopPrank();
    ...
    function testSuccessfulTerminateListing() public {
        ...
        purchaseToken.approve(address(secondaryMarket), 1e8);
        secondaryMarket.submitBid(address(collection), 1, 1e8, "Charles");
        assertEq(collection.balanceOf(address(secondaryMarket)), 1);
        ...
        secondaryMarket.terminateListing(address(collection), 1);
